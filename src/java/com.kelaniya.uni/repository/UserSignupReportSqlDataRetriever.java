...
public class UserSignupReportSqlDataRetriever extends SqlRepository implements DataRetriever {
    ...
    public ArrayList<String[]> retrieve(String startDate, String endDate){
        ...
        try {
            ...
            ResultSet resultset = statement.executeQuery("SELECT users.f_name AS Name, address.city AS City,address.district As District FROM users,address " +
                    "WHERE users.email=address.email AND users.signup_date BETWEEN '" + startDate + "' AND '" + endDate + "' ");
